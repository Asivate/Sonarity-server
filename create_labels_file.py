#!/usr/bin/env python3
"""
Create a JSON file with AudioSet labels for use with PANNs model.
This ensures we have a local copy of the labels even if the model doesn't provide them.
"""

import os
import json

# AudioSet labels from https://github.com/qiuqiangkong/audioset_tagging_cnn/blob/master/audioset_tagging_cnn/data_generator.py
AUDIOSET_LABELS = [
    ...
    'Wail, moan', 'Sigh', 'Singing', 'Choir', 'Yodeling', 'Chant',
    ...
    'Hiccup', 'Fart', 'Hands', 'Finger snapping', 'Clapping',
    ...
    'Yip', 'Howl', 'Bow-wow', 'Growling', 'Whimper (dog)', 'Cat',
    'Purr', 'Meow', 'Hiss', 'Caterwaul', 'Livestock, farm animals, working animals',
    'Horse', 'Clip-clop', 'Neigh, whinny', 'Cattle, bovinae',
    'Moo', 'Cowbell', 'Pig', 'Oink', 'Goat', 'Bleat', 'Sheep',
    'Fowl', 'Chicken, rooster', 'Cluck', 'Crowing, cock-a-doodle-doo',
    ...
    'Mosquito', 'Fly, housefly', 'Buzz', 'Bee, wasp, etc.',
    ...
    'Car', 'Vehicle horn, car horn, honking', 'Toot', 'Car alarm', 'Power windows, electric windows',
    ...
    'Engine', 'Light engine (high frequency)', 'Dental drill, dentist\'s drill',
    ...
    'Bathtub (filling or washing)', 'Hair dryer', 'Toilet flush', 'Toothbrush',
    ...
    'Steam whistle', 'Mechanisms', 'Ratchet, pawl', 'Clock',
    'Tick', 'Tick-tock', 'Gears', 'Pulleys', 'Sewing machine',
    'Mechanical fan', 'Air conditioning', 'Cash register',
    'Printer', 'Camera', 'Single-lens reflex camera', 'Tools',
    'Hammer', 'Jackhammer', 'Sawing', 'Filing (rasp)', 'Sanding',
    'Power tool', 'Drill', 'Explosion', 'Gunshot, gunfire',
    'Machine gun', 'Fusillade', 'Artillery fire', 'Cap gun',
    'Fireworks', 'Firecracker', 'Burst, pop', 'Eruption', 'Boom',
    ...
    'Spray', 'Pump (liquid)', 'Stir', 'Boiling', 'Sonar', 'Arrow',
    ...
    'Ping', 'Ding', 'Clang', 'Squeal', 'Creak', 'Rustle', 'Whir',
    ...
    'Pulse', 'Inside, small room', 'Inside, large room or hall',
    'Inside, public space', 'Outside, urban or manmade',
    'Outside, rural or natural', 'Reverberation', 'Echo',
    'Noise', 'Environmental noise', 'Static', 'Mains hum',
    'Distortion', 'Sidetone', 'Cacophony', 'White noise',
    'Pink noise', 'Throbbing', 'Vibration', 'Television',
    'Radio', 'Field recording'
]

def create_audioset_labels_file():
    """
    Create a JSON file containing AudioSet labels for use with the PANNs model.
    """
    home_dir = os.path.expanduser("~")
    panns_data_dir = os.path.join(home_dir, "panns_data")
    os.makedirs(panns_data_dir, exist_ok=True)
    
    # Save in both the current directory and the panns_data directory
    paths = [
        'audioset_labels.json',
        os.path.join(panns_data_dir, 'audioset_labels.json')
    ]
    
    for path in paths:
        with open(path, 'w') as f:
            json.dump(AUDIOSET_LABELS, f, indent=2)
        print(f"Created AudioSet labels file at: {path}")

if __name__ == "__main__":
    create_audioset_labels_file() 